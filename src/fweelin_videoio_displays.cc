...
void FloDisplayPanel::Draw(SDL_Renderer *renderer) {
    ...
    boxRGBA(renderer,
            ...
            0,0,0,190);
    vlineRGBA(renderer,xpos,ypos,ypos+sy,
              borderclr.r,borderclr.g,borderclr.b,255);
    vlineRGBA(renderer,xpos+sx,ypos,ypos+sy,
              borderclr.r,borderclr.g,borderclr.b,255);
    hlineRGBA(renderer,xpos,xpos+sx,ypos,
              borderclr.r,borderclr.g,borderclr.b,255);
    hlineRGBA(renderer,xpos,xpos+sx,ypos+sy,
              borderclr.r,borderclr.g,borderclr.b,255);
    ...
    // Draw title
    if (title != 0)
        VideoIO::draw_text(renderer,font->font,
                           title,xpos+sx-margin,ypos,titleclr,2,0);
}

void LoopTray::Draw(SDL_Renderer *renderer) {
    ...
    boxRGBA(renderer,xpos,ypos,xpos+iconsize,ypos+iconsize,
            borderclr.r,borderclr.g,borderclr.b,255);
    hlineRGBA(renderer,xpos,xpos+iconsize,ypos,40,40,40,255);
    hlineRGBA(renderer,xpos,xpos+iconsize,ypos+iconsize,40,40,40,255);
    vlineRGBA(renderer,xpos,ypos,ypos+iconsize,40,40,40,255);
    vlineRGBA(renderer,xpos+iconsize,ypos,ypos+iconsize,40,40,40,255);
    filledPieRGBA(renderer,xpos+iconsize/2,ypos+iconsize/2,
                  iconsize*3/8,
                  30,359,
                  0xF9,0xE6,0x13,255);
    circleRGBA(renderer,xpos+iconsize/2,ypos+iconsize/2,
               iconsize*3/8,
    ...
    if (xpanded) {
        // Draw background
        {
            ...
            boxRGBA(renderer,xpand_x1,xpand_y1,xpand_xb1,xpand_y2,
                    borderclr.r,borderclr.g,borderclr.b,255);
            boxRGBA(renderer,xpand_xb1,xpand_y1,xpand_x2,xpand_y2,
                    borderclr.r,borderclr.g,borderclr.b,255);
            boxRGBA(renderer,xpand_x1,xpand_y1,xpand_x2,xpand_yb1,
                    borderclr.r,borderclr.g,borderclr.b,255);
            boxRGBA(renderer,xpand_x1,xpand_yb2,xpand_x2,xpand_y2,
                    borderclr.r,borderclr.g,borderclr.b,255);
            boxRGBA(renderer,xpand_xb1,xpand_yb1,xpand_xb2,xpand_yb2,0,0,0,255);
        }

        hlineRGBA(renderer,xpand_x1,xpand_x2,xpand_y1,40,40,40,255);
        hlineRGBA(renderer,xpand_x1,xpand_x2,xpand_y2,40,40,40,255);
        vlineRGBA(renderer,xpand_x1,xpand_y1,xpand_y2,40,40,40,255);
        vlineRGBA(renderer,xpand_x2,xpand_y1,xpand_y2,40,40,40,255);
        ...
        while (curl != 0 && go) {
            if (curl->xpos != -1)
                Draw_Item(renderer,curl,xpand_x1+curl->xpos,xpand_y1+curl->ypos);
            else
                go = 0;
...
void LoopTray::Draw_Item(SDL_Renderer *renderer, BrowserItem *i, int x, int y) {
    ...
    if (loopexists) {
        if (!app->getVIDEO()->
                DrawLoop(loopmgr,li->loopid,renderer,app->getVIDEO()->getLSCOPEPIC(),
                         loop_color,colormag,app->getCFG(),0,loopmap,x,y,
                         app->getMASTERLIMITER()->GetLimiterVolume(),0)) {
            // Place name
            if (li->placename != 0)
                VideoIO::draw_text(renderer,font->font,li->placename,x,y,white);
            ...
            if (i == cur && renamer != 0) {
                ...
                if (*curn != '\0')
                    VideoIO::draw_text(renderer,font->font,curn,
                ...
                VideoIO::draw_text(renderer,font->font,li->name,
                                   x,y+loopsize-xpand_liney,
                                   white);
        }
    }

    // Browser::Draw_Item(renderer,i,x,y);
};

// Draw browser display
void Browser::Draw_Item(SDL_Renderer *renderer, BrowserItem *i, int x, int y) {
    const static SDL_Color white = { 0xEF, 0xAF, 0xFF, 0 };
    ...
    if (font != 0 && font->font != 0 && i != 0) {
        ...
        case B_Patch :
        {
            ...
            snprintf(tmp,255,"%02d: %s",
                     ...
                     p->name);
            tmp[254] = '\0';
            VideoIO::draw_text(renderer,font->font,tmp,x,y,white);
        }
        break;
        ...
        default :
        {
            ...
            if (i == cur && renamer != 0) {
                ...
                // Draw text with cursor
                int sx, sy;
                VideoIO::draw_text(renderer,font->font,tmp,x,y,white,0,0,
                                   &sx,&sy);
...
// Draw browser display
void Browser::Draw(SDL_Renderer *renderer) {
    LockBrowser();
    ...
    if (xpanded) {
        ...
        for (int i = xpand_y1; i <= xpand_y2; i++)
            hlineRGBA(renderer,xpand_x1,xpand_x2,i,0,0,0,200);
        hlineRGBA(renderer,xpand_x1,xpand_x2,xpand_y1,127,127,127,255);
        hlineRGBA(renderer,xpand_x1,xpand_x2,xpand_y2,127,127,127,255);
        vlineRGBA(renderer,xpand_x1,xpand_y1,xpand_y2,127,127,127,255);
        vlineRGBA(renderer,xpand_x2,xpand_y1,xpand_y2,127,127,127,255);
        ...
        if (cur != 0) {
            ...
            boxRGBA(renderer,
                    ...
                    127,0,0,255);
            Draw_Item(renderer,sp_1,xpand_x1,
            ...
            while (ofs_2 < xpand_spread-1 && sp_2->next != 0) {
                sp_2 = sp_2->next;
                ofs_2++;
                Draw_Item(renderer,sp_2,xpand_x1,
    ...
    Draw_Item(renderer,cur,xpos,ypos);

    UnlockBrowser();
}

void FloDisplayParamSet::Draw(SDL_Renderer *renderer) {
    const static SDL_Color titleclr = { 0x77, 0x88, 0x99, 0 };
    ...
    if (curbank < numbanks) {
        ...
        if (b->name != 0)
            VideoIO::draw_text(renderer,font->font,banks[curbank].name,
        ...
        for (int i = b->firstparamidx;
            ...
            if (b->params[i].name != 0)
                VideoIO::draw_text(renderer,font->font,b->params[i].name,
...
// Draw text display
void FloDisplayText::Draw(SDL_Renderer *renderer) {
    static char tmp[255];
    const static SDL_Color titleclr = { 0x77, 0x88, 0x99, 0 };
...
// Draw switch display
void FloDisplaySwitch::Draw(SDL_Renderer *renderer) {
...
void FloDisplayCircleSwitch::Draw(SDL_Renderer *renderer) {
    const static SDL_Color titleclr = { 0x77, 0x88, 0x99, 0 };
    ...
    filledCircleRGBA(renderer,xpos,ypos,(nonz && flashon ? rad1 : rad0),
                     c->r, c->g, c->b, 255);
...
void FloDisplayTextSwitch::Draw(SDL_Renderer *renderer) {
    // No title displayed
    ...
    if (dtxt != 0)
        VideoIO::draw_text(renderer,font->font,dtxt,
                           xpos,ypos,
                           (nonz ? c1clr : c0clr),0,1);
};

// Draw text display
void FloDisplayBar::Draw(SDL_Renderer *renderer) {
    const static SDL_Color titleclr = { 0x77, 0x88, 0x99, 0 };
    const static SDL_Color barclr = { 0xFF, 0x50, 0x20, 0 };
...
// Draw text display
void FloDisplayBarSwitch::Draw(SDL_Renderer *renderer) {
    const static SDL_Color titleclr = { 0x77, 0x88, 0x99, 0 },
                           warnclr = { 0xFF, 0, 0, 0 };
...
void FloLayoutBox::Draw(SDL_Renderer *renderer, SDL_Color clr) {
    // Solid box
    boxRGBA(renderer,
    ...
    // Outline
    if (lineleft)
        vlineRGBA(renderer,left,top,bottom,0,0,0,255);
    if (lineright)
        vlineRGBA(renderer,right,top,bottom,0,0,0,255);
    if (linetop)
        hlineRGBA(renderer,left,right,top,0,0,0,255);
    if (linebottom)
        hlineRGBA(renderer,left,right,bottom,0,0,0,255);
};

// Draw snapshots display
void FloDisplaySnapshots::Draw(SDL_Renderer *renderer) {
    const static SDL_Color titleclr = { 0x77, 0x88, 0x99, 0 };
    const static SDL_Color borderclr = { 0xFF, 0x50, 0x20, 0 };
    const static SDL_Color cursorclr = { 0xEF, 0x11, 0x11, 0 };
